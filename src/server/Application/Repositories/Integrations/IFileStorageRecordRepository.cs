...
public interface IFileStorageRecordRepository
{
    ...
    Task<DatabaseActionResult<PaginatedDbEntity<IEnumerable<FileStorageRecordDb>>>> GetAllPaginatedAsync(int pageNumber, int pageSize);
    ...
    Task<DatabaseActionResult<PaginatedDbEntity<IEnumerable<FileStorageRecordDb>>>> SearchPaginatedAsync(string searchTerm, int pageNumber, int pageSize);
