...
public interface INotifyRecordRepository
{
    ...
    Task<DatabaseActionResult<IEnumerable<NotifyRecordDb>>> GetAllPaginatedAsync(int pageNumber, int pageSize);
    ...
    Task<DatabaseActionResult<IEnumerable<NotifyRecordDb>>> SearchPaginatedAsync(string searchTerm, int pageNumber, int pageSize);
